...
from backend.database.crud import create_user, create_tenant
...
class RegisterRequest(BaseModel):
    ...
    full_name: str = ""
    role: str = "analyst"
    # An organization id is deliberately not accepted from the browser.  The
    # server assigns it for self-signup, while admins can only add users to
    # their own authenticated tenant.
...
def login(
    ...
    token = create_access_token(data={"sub": user.username, "role": user.role, "tenant_id": user.tenant_id or "tenant-default"})
...
def signup(
    ...
    create_tenant(
        db,
        tenant_id=tenant_id,
        name=(payload.tenant_name or f"{payload.full_name or payload.username} Organization").strip()[:128],
    )
    user = create_user(
        db=db,
    ...
    token = create_access_token(data={"sub": user.username, "role": user.role, "tenant_id": user.tenant_id})
