...
def seed_demo_users(db: Session) -> None:
    """
    Create default tenant and demo users on first run if they don't already exist.
    Called during application startup.
    """
    from backend.database.models import Tenant
    default_tenant = db.query(Tenant).filter_by(id="tenant-default").first()
    if default_tenant is None:
        default_tenant = Tenant(id="tenant-default", name="CampusShield Enterprise Tenant", plan="enterprise")
    ...
    for user_data in DEMO_USERS:
        ...
        if existing is None:
            create_user(
                ...
                role=user_data["role"],
                tenant_id=user_data.get("tenant_id", "tenant-default"),
            )
