...
class Tenant(Base):
    """
    SaaS Tenant entity representing an organization/customer workspace.
    Guarantees strict isolation between Tenant A and Tenant B data.
    """
    __tablename__ = "tenants"

    id = Column(String(64), primary_key=True, default="tenant-default")
...
class User(Base):
    ...
    tenant_id = Column(String(64), ForeignKey("tenants.id"), nullable=False, default="tenant-default", index=True)
    ...
    def __repr__(self) -> str:
        return f"<User(id={self.id}, tenant_id='{self.tenant_id}', username='{self.username}', role='{self.role}')>"
...
class DetectionResult(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    # Kept on the result as well as its session to make tenant-scoped exports
    # and the future DynamoDB mapping explicit.  Route access still verifies
    # ownership through the parent session.
    tenant_id = Column(String(64), ForeignKey("tenants.id"), nullable=False, default="tenant-default", index=True)
...
class Alert(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_alerts_tenant", "tenant_id"),
        UniqueConstraint("session_id", "detection_result_id", name="uq_alert_session_detection"),
    )
