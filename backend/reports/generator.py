...
def generate_report(
    ...
    """
    # Fetch data
    session = crud.get_session(db, session_id, tenant_id=tenant_id)
    if session is None:
        raise ValueError(f"Session {session_id} not found")

    results = crud.get_detection_results_for_session(db, session_id, tenant_id=tenant_id)
    alerts = crud.list_alerts(db, session_id=session_id, limit=100, tenant_id=tenant_id)
    ...
    if not HAS_REPORTLAB:
        ...
        content_str = (
            ...
            f"Total Windows: {total_w}\nAnomalous Windows: {anom_w}\n"
            f"Generated: {datetime.now(timezone.utc)}\n"
        )
        storage_svc = get_storage_service()
        saved_path = storage_svc.save_file(content_str.encode("utf-8"), filename, category="reports")
