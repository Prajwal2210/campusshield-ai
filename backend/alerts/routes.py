...
def get_alert_stats(
    ...
    return crud.get_alert_stats(db, tenant_id=current_user.tenant_id)
...
def get_alert(
    ...
    alert = crud.get_alert(db, alert_id, tenant_id=current_user.tenant_id)
    if alert is None:
...
def acknowledge_alert(
    ...
    alert = crud.acknowledge_alert(
        db, alert_id, current_user.username, tenant_id=current_user.tenant_id
    )
    if alert is None:
        raise HTTPException(status_code=404, detail="Alert not found")
