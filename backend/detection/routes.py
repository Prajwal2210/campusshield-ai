...
def get_config(
    ...
    config = crud.get_detection_config(db, tenant_id=current_user.tenant_id)
...
def update_config(
    ...
    config = crud.update_detection_config(db, tenant_id=current_user.tenant_id, **update_data)
...
def get_detection_results(
    ...
    if crud.get_session(db, session_id, tenant_id=current_user.tenant_id) is None:
        raise HTTPException(status_code=404, detail="Session not found")
    results = crud.get_detection_results_for_session(db, session_id, tenant_id=current_user.tenant_id)
    return [
...
def get_anomalous_results(
    ...
    if crud.get_session(db, session_id, tenant_id=current_user.tenant_id) is None:
        raise HTTPException(status_code=404, detail="Session not found")
    results = crud.get_anomalous_results_for_session(db, session_id, tenant_id=current_user.tenant_id)
    return [
        DetectionResultResponse(
            id=r.id,
            session_id=r.session_id,
...
def get_contributing_factors(
    ...
    # Ownership is resolved through the result's parent session rather than a
    # browser-supplied tenant identifier.
    from backend.database.models import DetectionResult
    result = db.query(DetectionResult).filter(DetectionResult.id == detection_id).first()
    if result is None or crud.get_session(db, result.session_id, tenant_id=current_user.tenant_id) is None:
        raise HTTPException(status_code=404, detail="Detection result not found")
    factors = crud.get_factors_for_detection(db, detection_id, tenant_id=current_user.tenant_id)
    return [
