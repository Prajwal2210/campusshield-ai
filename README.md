...
### Production-ready local configuration

CampusShield uses Neon PostgreSQL as its active database. The historical
`backend/data/campusshield.db` is preserved only as legacy/reference data and
is never opened by the application.

Create a Neon database, copy `.env.example` to `.env`, and set:

```env
DATABASE_URL=postgresql+psycopg://USER:PASSWORD@YOUR_NEON_HOST/DBNAME?sslmode=require
JWT_SECRET_KEY=a-long-random-secret
```

Optionally provide all `INITIAL_ADMIN_*` values in `.env` to seed one admin on
first startup. Otherwise use the existing authenticated self-signup endpoint.
Credentials and secrets are never hardcoded in source.

The backend creates the schema from SQLAlchemy models on startup. It includes
tenants, users, traffic sessions, detection results, contributing factors,
alerts, report metadata, detection configuration, and audit logs. Every
tenant-owned API query is constrained to the tenant from the validated JWT.

For containers, run `docker compose up --build`; no local PostgreSQL or cloud
resources are created. The frontend is served on port 5173 and proxies API and
WebSocket traffic to the FastAPI backend on port 8000.
...
Use the Sign Up tab to create an analyst account, or use credentials supplied
by the deployment administrator. An administrator may be bootstrapped only by
setting the optional `INITIAL_ADMIN_*` environment variables before first run.
