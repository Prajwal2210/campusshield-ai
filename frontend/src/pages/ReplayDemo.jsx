...
import { simulateTraffic, getScenarios, getDetectionResults, getAlerts } from '../services/api';
...
export default function ReplayDemo({ wsAlerts, wsProgress }) {
  ...
  const loadResults = async (sid) => {
    try {
      ...
      // Fetch persisted alerts for this session — feeds Incident Feed
      try {
        const alertRes = await getAlerts({ session_id: sid });
        setSessionAlerts(alertRes.data || []);
      } catch (e) {
        // Fall back to WebSocket alerts already in memory
        setSessionAlerts(wsAlerts?.filter((a) => a.session_id === sid) || []);
      }

      // Notify other pages (Reports) that a new completed session exists
      window.dispatchEvent(new CustomEvent('campusshield:session_completed', {
