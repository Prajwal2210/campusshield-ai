...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedErr = typeof detail === 'string' 
        ? detail 
        : Array.isArray(detail) 
          ? detail.map(d => d.msg || JSON.stringify(d)).join(', ') 
          : (isSignUp ? 'Registration failed. Username may already exist.' : 'Invalid credentials. Access restricted.');
    ...
    <div className="login-page">
      <div className="login-card animate-in" style={{ maxWidth: '440px' }}>
        ...
        <form className="login-form" onSubmit={handleSubmit} autoComplete="off">
          {/* Hidden honeypot inputs — prevent browser autofill from targeting the real fields.
              Chrome and other browsers fill these dummy fields instead of the visible ones. */}
          <input type="text" name="username_fake" style={{ display: 'none' }} aria-hidden="true" tabIndex={-1} readOnly />
          <input type="password" name="password_fake" style={{ display: 'none' }} aria-hidden="true" tabIndex={-1} readOnly />
