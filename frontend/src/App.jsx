    ...
    <div className="top-telemetry-bar" style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span className="telemetry-tag" style={{ color: 'var(--text-primary)', background: 'var(--bg-surface)' }}>
          👤 {user?.username || 'Authenticated User'} ({user?.role?.toUpperCase() || 'USER'})
        </span>
      </div>
...
function ProtectedLayout({ user, onLogout, theme, onToggleTheme }) {
  ...
  // Listen for WebSocket alerts and add them to the notification panel
  useEffect(() => {
    if (alerts && alerts.length > 0) {
      const latest = alerts[0];
      // Avoid duplicate notifications for the same alert id
      setNotifications((prev) => {
        const alreadyExists = prev.some((n) => n.wsAlertId === latest.id);
        ...
        const color = severityColors[latest.severity] || '#00f0ff';
        const newNotif = {
          id: Date.now(),
          wsAlertId: latest.id,
          title: `${latest.severity} Alert — ${latest.threat_category || 'Anomaly'}`,
          message: `Score: ${latest.threat_score?.toFixed(1)}/100 | Session #${latest.session_id}`,
          color,
          time: new Date().toUTCString().slice(17, 25) + ' UTC',
          isDanger: latest.severity === 'CRITICAL' || latest.severity === 'HIGH',
          read: false,
        };
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        <Routes>
          ...
          />
          {/* Pass wsProgress so these pages can refresh when a pipeline completes */}
          <Route path="/alerts" element={<AlertsPage wsProgress={progress} />} />
          <Route path="/reports" element={<Reports wsProgress={progress} />} />
...
export default function App() {
  ...
  // authChecked prevents rendering routes before token validation completes.
  // Without this guard, pages load and call /api/alerts with a stale token
  // (from a previous signup session) before getMe() has a chance to clear it.
  const [authChecked, setAuthChecked] = useState(false);

  // On every app load, validate the stored JWT against /api/auth/me.
  // - Valid token belonging to the stored user → continue session.
  // - Invalid/stale/wrong-tenant token → clear localStorage, redirect to /login.
  ...
  useEffect(() => {
    ...
    getMe()
      .then((res) => {
        ...
        // Check if the token belongs to the same user that is stored locally.
        // If someone stored a token from a different account (e.g. a test signup),
        // the stored username won't match and we force re-login so the correct
        // tenant's data is loaded instead of an empty tenant's data.
        let storedUsername = null;
        try {
          const stored = localStorage.getItem('campusshield_user');
          if (stored) storedUsername = JSON.parse(stored)?.username;
        } catch (_) {}

        if (storedUsername && storedUsername !== freshUser.username) {
          // Token is valid but belongs to a different account — force re-login
          localStorage.removeItem('campusshield_token');
          localStorage.removeItem('campusshield_user');
          setUser(null);
          setIsAuthenticated(false);
        } else {
          // Token is valid and matches the stored user — refresh profile
          localStorage.setItem('campusshield_user', JSON.stringify(freshUser));
          setUser(freshUser);
          setIsAuthenticated(true);
        }
      })
      .catch(() => {
        // Stale/expired/revoked token — force re-login
        localStorage.removeItem('campusshield_token');
        localStorage.removeItem('campusshield_user');
        setUser(null);
        setIsAuthenticated(false);
  ...
  // Don't render routes until auth check is complete — avoids flash of
  // wrong-tenant data while getMe() is in flight.
  if (!authChecked) {
    return (
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        height: '100vh', background: 'var(--bg-base)',
        color: 'var(--accent-cyan)', fontFamily: 'var(--font-mono)', fontSize: '13px',
      }}>
